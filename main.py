...
def main():
    ...
    print("\n=== DCD L1-SVM ===")
    model_l1 = LinearSVM(loss="l1", C=C, max_iter=5000, tol=1e-4, permute=False, verbose=False)
    t0 = time.time()
    ...
    print("\n=== DCD L2-SVM ===")
    model_l2 = LinearSVM(loss="l2", C=C, max_iter=5000, tol=1e-4, permute=False, verbose=False)
    t0 = time.time()
    ...
    print(f"  Time: {t_skl:.3f}s | Iter: {skl.n_iter_} | Acc: {acc_skl:.4f}")

    print("\n=== sklearn LinearSVC (L2 hinge) ===")
    skl = LinearSVC(loss="squared_hinge", C=C, max_iter=5000, dual=True)
    t0 = time.time()
    skl.fit(X_train, y_train)
    t_skl = time.time() - t0
    acc_skl = accuracy_score(y_test, skl.predict(X_test))
    ...
    print(f"{'sklearn LinearSVC L1':<25} {t_skl:>10.3f} {skl.n_iter_:>10} {acc_skl:>10.4f}")
    print(f"{'sklearn LinearSVC L2':<25} {t_skl:>10.3f} {skl.n_iter_:>10} {acc_skl:>10.4f}")
